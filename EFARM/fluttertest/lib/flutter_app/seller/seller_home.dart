...
import 'package:fluttertest/flutter_app/seller/addproduct.dart';
import 'package:fluttertest/flutter_app/seller/chat_page.dart';
...
//class MyApp extends StatelessWidget {
 //final int userId;
  //const SellerHome({super.key, required this.userId,});

  //@override
  //Widget build(BuildContext context) {
    //return MaterialApp(
      //debugShowCheckedModeBanner: false,
      //home: SellerHome(userId: widget.userId),
    //);
  //}
//}
...
class SellerHome extends StatefulWidget {
  ...
  const SellerHome({super.key, required this.userId,required this.userName, required this.userEmail,});
...
class _SellerHomeState extends State<SellerHome> {
  ...
  Widget build(BuildContext context) {
    List<Widget> pages = [
      ...
      SellerOrders(userId: widget.userId,),
...
class Product extends StatelessWidget {
  final int userId;
  const Product({required this.userId,});

  @override
  Widget build(BuildContext context) {
  final int userId;
    return MaterialApp(
      home: ProductPage(userId: this.userId),
    );
  }
}
...
//  TextEditingController emailController = TextEditingController();
  ...
  Future fetchProducts() async {
    final response =
    await http.get(Uri.parse("$baseUrl/get_productso.php?user_id=${widget.userId}"));
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed:() {
          ...
          Navigator.push(context,
            MaterialPageRoute(builder: (context) => AddProductPage(userId: widget.userId)),
          );
          //     _incrementCounter,
          //   tooltip: 'Increment',
        },
...
class SellerOrders extends StatefulWidget {
 // final int productId;
  final int userId;
  SellerOrders({super.key, required this.userId, });
...
class _SellerOrdersState extends State<SellerOrders> {
  ...
  fetchOrders() async {
    final response = await http.post(
      Uri.parse("http://10.0.2.2/backend/get_seller_orders.php"),
      body: jsonEncode({"product_id": widget.userId}),
    );
